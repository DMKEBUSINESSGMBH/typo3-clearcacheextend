...
class tx_clearcacheextend
{
                    ...
                    if (!$this->error) {
                        if (count($this->minusArray) > 0) {
                            foreach ($this->minusArray as $minusArr) {
                                if ($this->plussArray[$minusArr] == $minusArr) {
                                    unset($this->plussArray[$minusArr]);
                                }
                            };
                        };
    ...
    protected function hasChanged($objArray, $string)
    {
		...
		*/
        ...
        if ($this->debugMode) {
            tx_rnbase_util_Logger::devLog(
                str_replace('###table###', $command, $GLOBALS['LANG']->getLL('syntax_changed_table')),
                'clearcacheextend',
                0
            );
            if ($this->showDebug) {
            ...
            if ($newArray[1] != 'none') {
                tx_rnbase_util_Logger::devLog(
                    $GLOBALS['LANG']->getLL('syntax_changed_truecommand').$newArray[1],
                    'clearcacheextend',
                    0
                );
                if ($this->showDebug) {
            ...
            if ($newArray[3] != 'none') {
                tx_rnbase_util_Logger::devLog(
                    $GLOBALS['LANG']->getLL('syntax_changed_falsecommand').$newArray[3],
                    'clearcacheextend',
                    0
                );
                if ($this->showDebug) {
    ...
    protected function getCommandArray($string)
    {
        ...
        if ($commandEnde === false || $commandEnde === 0 || substr($string, -1) != ')') {
            $this->error = true;
        ...
        switch ($command) {
            case ('pages'):{
                ...
                if (!is_array($innerArray)) {
                    return $this->makeDebug('syntax_error_pages_command_not_define_correctly');
                }
            ...
            case ('contains'):{
                ...
                switch ($containType) {
                    case ('plugin'):{
                        ...
                        if ($commandArrayIntern['contains']['plugin'] == '') {
                            return $this->makeDebug('syntax_error_contains_second_parameter_missing');
                        };
            ...
            case ('alias'):{
                if ($innerCommand == '') {
                    return $this->makeDebug('syntax_error_empty_alias');
                };
